...
class ibase_log_stream {
 public:
  ibase_log_stream() {
  }
  virtual ~ibase_log_stream() {
  }
  virtual bool out_buffer(const char* buffer, int buffer_len,
                          const char* plog_name = NULL)=0;
  virtual int get_type() {
    return 0;
  }

  virtual bool set_max_logfile_size(boost::uint64_t max_size) {
    return true;
  }

  virtual bool set_log_rotate_cmd(const std::string& cmd) {
    return true;
  }

};

class ibase_log_value {
 public:
  virtual void debug_out(std::stringstream* p_stream) const = 0;
  virtual ~ibase_log_value() {
  }
};
...
inline log_space::log_message& operator<<(log_space::log_message& sstream,
                                          const ibase_log_value& log_val) {
 ...
 public:
  ...
  virtual bool out_buffer(const char* buffer, int buffer_len,
    ...
    if (plog_name)
      ...
      boost::uint32_t b = 0;
      ::WriteConsoleA(::GetStdHandle(STD_OUTPUT_HANDLE), ptarget_buf,
                      buffer_len, (DWORD*)&b, 0);
...
class file_output_stream : public ibase_log_stream {
 public:
  ...
  file_output_stream(std::string default_log_file_name, std::string log_path) {
    ...
    m_pdefault_file_stream = add_new_stream_and_open(
        default_log_file_name.c_str());
  }

  ~file_output_stream() {
    for (named_log_streams::iterator it = m_log_file_names.begin();
        it != m_log_file_names.end(); it++) {
 ...
 private:
  ...
  std::ofstream* add_new_stream_and_open(const char* pstream_name) {
    ...
    std::ofstream* pstream =
        (m_log_file_names[pstream_name] = new std::ofstream);
    if (!pstream)
      return NULL;  //TODO: throw exception here or assert
    std::string target_path = m_default_log_path + "/" + pstream_name;
    pstream->open(target_path.c_str(),
                  std::ios_base::out | std::ios::app /*ios_base::trunc */);
    if (pstream->fail())
      return NULL;
    return pstream;
  }
  ...
  virtual bool out_buffer(const char* buffer, int buffer_len,
    ...
    if (plog_name) {  //find named stream
    ...
    if (m_max_logfile_size) {
      ...
      if (current_sz > m_max_logfile_size) {
        std::cout << "current_sz= " << current_sz << " m_max_logfile_size= "
                  << m_max_logfile_size << std::endl;
        ...
        do {
          ...
          if (err_count)
            new_log_file_name += misc_utils::get_time_str_v2(tm) + "("
                + boost::lexical_cast<std::string>(err_count) + ")" + ".log";
          else
            new_log_file_name += misc_utils::get_time_str_v2(tm) + ".log";

          err_count++;
        } while (boost::filesystem::exists(
            m_default_log_path + "/" + new_log_file_name, ec));

        std::string new_log_file_path = m_default_log_path + "/"
            + new_log_file_name;
        boost::filesystem::rename(m_default_log_path + "/" + log_file_name,
                                  new_log_file_path, ec);
        if (ec) {
        ...
        if (m_log_rotate_cmd.size()) {
          ...
          boost::replace_all(m_log_rotate_cmd_local_copy, "[*TARGET*]",
                             new_log_file_path);

          misc_utils::call_sys_cmd(m_log_rotate_cmd_local_copy);
        }

        m_target_file_stream->open(
            (m_default_log_path + "/" + log_file_name).c_str(),
            std::ios_base::out | std::ios::app /*ios_base::trunc */);
...
class log_stream_splitter {
 public:
  ...
  bool set_max_logfile_size(boost::uint64_t max_size) {
    for (streams_container::iterator it = m_log_streams.begin();
        it != m_log_streams.end(); it++)
  ...
  bool do_log_message(const log_message& rlog_mes,
                      const char* plog_name = NULL) {
  ...
  bool add_logger(int type, const char* pdefault_file_name,
                  const char* pdefault_log_folder) {
...
/*                                                                      */
/************************************************************************/
inline int get_set_log_detalisation_level(bool is_need_set = false,
                                          int log_level_to_set = LOG_LEVEL_1);
inline int get_set_time_level(bool is_need_set = false, int time_log_level =
                                  LOG_LEVEL_0);
inline bool get_set_need_thread_id(bool is_need_set = false, bool is_need_val =
                                       false);
inline bool get_set_need_proc_name(bool is_need_set = false, bool is_need_val =
                                       false);
...
class logger {
 public:
  ...
  bool do_log_message(const log_message& rlog_mes, bool add_to_journal = false,
                      const char* plog_name = NULL) {
    CRITICAL_REGION_BEGIN(m_critical_sec);
  ...
  bool add_logger(int type, const char* pdefault_file_name,
                  const char* pdefault_log_folder) {
    CRITICAL_REGION_BEGIN(m_critical_sec);
    return m_log_target.add_logger(type, pdefault_file_name,
                                   pdefault_log_folder);CRITICAL_REGION_END();
  }
  bool add_logger(ibase_log_stream* pstream) {
    CRITICAL_REGION_BEGIN(m_critical_sec);
    return m_log_target.add_logger(pstream);CRITICAL_REGION_END();
  }

  bool remove_logger(int type) {
    CRITICAL_REGION_BEGIN(m_critical_sec);
    return m_log_target.remove_logger(type);CRITICAL_REGION_END();
  }
 ...
 private:
  bool init() {
...
#ifdef WINDOWS_PLATFORM
    if(reg_utils::RegGetANSIString(HKEY_LOCAL_MACHINE, RK_LOG_DEFAULT_PATH_KEY,
            RK_LOG_DEFAULT_PATH_VAL,
    ...
    if(reg_utils::RegGetPODValue(HKEY_LOCAL_MACHINE, RK_LOG_SETTINGS_KEY,
            RK_LOG_DETAIL_LEVEL, detail_level))
    get_set_log_detalisation_level(true, detail_level);

    if(!reg_utils::RegGetPODValue(HKEY_LOCAL_MACHINE, RK_LOG_SETTINGS_KEY,
            RK_LOG_OPTIONS_VAL_NAME, val))
    val = LOG_DEF_OPTIONS_SET;

#endif

    get_set_need_proc_name(true,
                           val & MISC_LOG_DEBUG_LOG_PROCID ? true : false);
    get_set_need_thread_id(true,
                           val & MISC_LOG_DEBUG_LOG_THREADID ? true : false);

    // init default set of loggers
    init_default_loggers(val);

    this->do_log_message(
        log_space::log_message() << get_time_string() << " Init logging. Level="
  ...
  bool init_default_loggers(boost::uint32_t log_options) {
    ...
    if ((log_options & MISC_LOG_DEBUG_FORCE_LOG_DIRECT)
        || (log_options & MISC_LOG_DEBUG_WRITE_LOGFILES)) {
#ifdef LOG_DEFAULT_FILENAME
      add_logger(LOGGER_FILE, LOG_DEFAULT_FILENAME, m_default_log_folder.c_str());
#else
      add_logger(LOGGER_FILE, m_default_log_file.c_str(),
                 m_default_log_folder.c_str());
#endif
    }
...
class log_singletone {
 public:
  ...
  static bool do_log_message(log_message& rlog_mes, bool keep_in_journal,
                             const char* plog_name = NULL) {
    logger* plogger = get_or_create_instance();
    bool res = false;
    if (plogger)
      res = plogger->do_log_message(rlog_mes, keep_in_journal, plog_name);
    else {  //globally uninitialized, create new logger for each call of do_log_message() and then delete it
      plogger = new logger();
  ...
  static bool add_logger(int type, const char* pdefault_file_name,
                         const char* pdefault_log_folder) {
    logger* plogger = get_or_create_instance();
    if (!plogger)
      return false;
  ...
  static int get_set_log_detalisation_level(
      bool is_need_set = false, int log_level_to_set = LOG_LEVEL_1) {
  ...
  static int get_set_time_level(bool is_need_set = false, int time_log_level =
                                    LOG_LEVEL_0) {
  ...
  static int get_set_process_level(bool is_need_set = false,
                                   int process_log_level = LOG_LEVEL_0) {
  ...
  static bool get_set_need_thread_id(bool is_need_set = false,
                                     bool is_need_val = false) {
  ...
  static bool get_set_need_proc_name(bool is_need_set = false,
                                     bool is_need_val = false) {
  ...
  static boost::uint64_t get_set_err_count(bool is_need_set = false,
                                           boost::uint64_t err_val = false) {
...
#pragma pack(pop)
  ...
  static std::string get_prefix_entry() {
    ...
    logger* plogger = get_or_create_instance();
//    bool res = false;
    if (!plogger) {  //globally uninitialized, create new logger for each call of get_prefix_entry() and then delete it
      plogger = new logger();
    }
...
//#endif
    ...
    if (plogger->m_thr_prefix_strings.size()) {
      ...
      std::map<std::string, std::string>::iterator it = plogger
          ->m_thr_prefix_strings.find(thr_str);
 ...
 private:
  ...
  static logger* get_set_instance_internal(bool is_need_set = false,
                                           logger* pnew_logger_val = NULL) {
  ...
  static bool get_set_is_uninitialized(bool is_need_set = false,
...
class log_array_value {
 ...
 public:
  ...
  log_array_value(log_message& log_mes)
      : num(0),
        m_ref_log_mes(log_mes) {
  }
...
class log_frame {
 ...
 public:
  ...
  log_frame(const log_message& name, int dlevel = LOG_LEVEL_2,
            const char* plog_name = NULL) {
#ifdef WINDOWS_PLATFORM
    ...
    if (dlevel <= log_singletone::get_log_detalisation_level()) {
      m_name = name.str();
      log_singletone::do_log_message(
          log_message() << log_space::log_singletone::get_prefix_entry()
                        << "-->>" << m_name << "\n",
          false, m_plog_name);
    }
    m_level = dlevel;
...
inline int get_set_log_detalisation_level(bool is_need_set,
                                          int log_level_to_set) {
  return log_singletone::get_set_log_detalisation_level(is_need_set,
                                                        log_level_to_set);
}
...
#define DEFINE_BASE_LOGGING() \
  initializer<epee::log_space::log_singletone> epee::log_space::log_singletone::m_log_initializer;

#define LOG_PRINT_NO_PREFIX2(log_name, x, y) \
  {if ( y <= epee::log_space::log_singletone::get_log_detalisation_level() )\
  {epee::log_space::log_singletone::do_log_message(epee::log_space::log_message()\
  << x << "\n", false, log_name);}}
...
#define LOG_PRINT_NO_POSTFIX2(log_name, x, y) \
    {if ( y <= epee::log_space::log_singletone::get_log_detalisation_level() )\
    {epee::log_space::log_singletone::do_log_message(epee::log_space::log_message()\
    << epee::log_space::log_singletone::get_prefix_entry() << x, false, log_name);}}

#define LOG_PRINT2(log_name, x, y) \
  { if (y <= epee::log_space::log_singletone::get_log_detalisation_level())\
	{epee::log_space::log_singletone::do_log_message(epee::log_space::log_message()\
	<< epee::log_space::log_singletone::get_prefix_entry() \
	<< x << "\n", false, log_name);}}
...
#define LOG_ERROR2(log_name, x) \
    {epee::log_space::log_singletone::do_log_message(epee::log_space::log_message() \
     ...
     << __LINE__ \
     <<	" " \
     << x << "\n", true, log_name);LOCAL_ASSERT(0);\
     log_space::log_singletone::get_set_err_count(true,\
           log_space::log_singletone::get_set_err_count()+1);}

#define LOG_FRAME2(log_name, x, y)\
    epee::log_space::log_frame frame(epee::log_space::log_message() << x, y, log_name)
...
// akrutogolov
#define LOG(msg)  LOG_PRINT_L0(msg)
