...
namespace net_utils
{
	namespace http
	{
		...
		//--------------------------------------------------------------------------------------------
		inline 
		simple_http_connection_handler::simple_http_connection_handler(i_send_handler* psnd_hndlr, config_type& config):
  ...
  //--------------------------------------------------------------------------------------------
    ...
    LOG_PRINT_L3("HTTP_RESPONSE_HEAD: << \r\n" << response_data);
		...
		m_psnd_hndlr->handle_send((void*)response_data.data(), response_data.size());
		if(response.m_body.size())
			m_psnd_hndlr->handle_send((void*)response.m_body.data(), response.m_body.size());
		return res;
	}
	//-----------------------------------------------------------------------------------
